...
public class TwoLaneCop : MonoBehaviour
{
    ...
    private List<WaypointNode> currentPath = new List<WaypointNode>(); //  to store A* path
    private int currentPathIndex = 0;       // to track which node in the path we are currently moving toward

    private WaypointNode lastGoalNode;     // Stores previous goal node to prevent unnecessary repathing
    ...
    void FixedUpdate()
    {
        ...
        // Repathing
        WaypointNode currentGoalNode = GetClosestNode(target.position); // Determine nearest node to player
    ...
    WaypointNode GetClosestNode(Vector3 position)
    {
        ...
        {
            WaypointNode node = child.GetComponent<WaypointNode>(); // Get node component
            if (!node)
                continue;
    ...
    List<WaypointNode> FindPath(WaypointNode start, WaypointNode goal)
    {
        ...
        HashSet<WaypointNode> closedSet = new HashSet<WaypointNode>(); // To prevent revisiting nodes and avoid infinite loops
        ...
        Dictionary<WaypointNode, float> gScore = new Dictionary<WaypointNode, float>(); // to represent the real travel distance so far
    ...
    float GetFScore(WaypointNode node, WaypointNode goal, Dictionary<WaypointNode, float> gScore)
    {
